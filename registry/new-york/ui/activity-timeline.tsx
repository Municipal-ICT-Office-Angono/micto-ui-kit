...
export type TimelineStatus = "completed" | "in-progress" | "pending" | "error" | "muted";
    ...
    return (
      <ActivityTimelineContext.Provider value={{ orientation }}>
        <div
          ...
          className={cn(
            "relative flex",
            orientation === "vertical" ? "flex-col" : "flex-row w-full gap-4 overflow-x-auto pb-4",
            className
          )}
          {...props}
        >
...
export const ActivityTimelineItem = React.forwardRef<HTMLDivElement, ActivityTimelineItemProps>(
  ({ className, status = "pending", isLast = false, children, ...props }, ref) => {
      ...
      <TimelineItemContext.Provider value={{ status, isLast }}>
        <div
          ...
          className={cn(
            ...
            orientation === "vertical"
              ? "flex flex-row gap-4"
              : "flex-1 flex flex-col items-center min-w-[200px]",
            className
          )}
          {...props}
        >
...
export const ActivityTimelineTrack = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
      ...
      <div
        ...
        className={cn(
          orientation === "vertical"
            ...
            : "flex flex-row items-center justify-center relative w-full mb-3 h-8",
          className
        )}
        {...props}
      >
...
export const ActivityTimelineConnector = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    const { orientation } = React.useContext(ActivityTimelineContext);
      ...
      <div
        ...
        className={cn(
          ...
          orientation === "vertical"
            ...
            : "left-[calc(50%+16px)] right-[calc(-50%+16px)] top-1/2 -translate-y-1/2 h-[2px] mx-1.5",
          status === "completed" ? "bg-primary" : "bg-border",
          status === "in-progress" ? "bg-primary/50 bg-[length:200%_200%] animate-pulse" : "",
          status === "pending" || status === "muted" ? "border-l-2 border-dashed border-border/60 bg-transparent" : "",
          className
        )}
        {...props}
      />
...
export const ActivityTimelineDot = React.forwardRef<HTMLDivElement, ActivityTimelineDotProps>(
  ({ className, customDot, children, ...props }, ref) => {
    const { status } = React.useContext(TimelineItemContext);

    if (customDot) {
      return (
        <div ref={ref} className={cn("relative z-10 flex items-center justify-center", className)} {...props}>
          {customDot}
        </div>
      );
    }
    ...
    return (
      <div
        ...
        className={cn(
          ...
          status === "in-progress" && "border-primary ring-4 ring-primary/20 bg-background text-primary animate-pulse glow",
      ...
      >
        {children ?? (
          <>
            {status === "completed" && <Check className="size-4.5 stroke-[2.5]" />}
            {status === "in-progress" && <Clock className="size-4 stroke-[2]" />}
            {status === "error" && <AlertCircle className="size-4.5 stroke-[2.5]" />}
            {status === "pending" && <Circle className="size-3.5 fill-muted-foreground/30" />}
...
export const ActivityTimelineContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => {
    const { orientation } = React.useContext(ActivityTimelineContext);
      ...
      <div
        ...
        className={cn(
          orientation === "vertical"
            ...
            : "flex flex-col items-center text-center w-full px-2 space-y-2",
          className
        )}
        {...props}
      >
...
export const ActivityTimelineList = React.forwardRef<HTMLDivElement, ActivityTimelineListProps>(
  ({ items, orientation = "vertical", className, ...props }, ref) => {
    return (
      <ActivityTimeline orientation={orientation} className={className} ref={ref} {...props}>
        {items.map((item, index) => (
          ...
          >
            ...
            <ActivityTimelineContent>
              ...
              <div className={cn("flex gap-2 justify-between items-start", orientation === "horizontal" && "flex-col items-center justify-center w-full")}>
                <div className={cn("space-y-0.5", orientation === "horizontal" && "items-center text-center")}>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex items-center gap-2 cursor-pointer rounded-full bg-muted/40 p-1 pr-2.5 border border-border/40 hover:bg-muted/70 transition-colors">
                          <Avatar size="sm">
                            <AvatarImage src={item.actor.avatar} alt={item.actor.name ?? "Actor"} />
                            <AvatarFallback>{item.actor.name?.[0] ?? "U"}</AvatarFallback>
                          </Avatar>
                          <div className="flex flex-col text-left">
                            <span className="text-xs font-medium text-foreground leading-none">{item.actor.name}</span>
                            {item.actor.role && <span className="text-[10px] text-muted-foreground">{item.actor.role}</span>}
                          </div>
                        </div>
                      </TooltipTrigger>
              ...
              {item.actor && orientation === "horizontal" && (
                <div className="flex items-center gap-1.5 mt-2 bg-muted/30 px-2 py-1 rounded-full border border-border/30">
                  <Avatar size="sm" className="size-4.5">
                    ...
                    <AvatarFallback className="text-[9px]">{item.actor.name?.[0] ?? "U"}</AvatarFallback>
                  </Avatar>
              ...
              {item.metadata && item.metadata.length > 0 && (
                <div className={cn("flex flex-wrap gap-1.5 pt-1", orientation === "horizontal" && "justify-center")}>
              ...
              {item.attachments && item.attachments.length > 0 && (
                <div className={cn("flex flex-wrap gap-2 pt-1", orientation === "horizontal" && "justify-center")}>
                  {item.attachments.map((att, aIdx) => (
                    <a
                      key={aIdx}
                      href={att.url ?? "#"}
                      onClick={att.onClick ? (e) => { e.preventDefault(); att.onClick?.(); } : undefined}
                    ...
                    >
                      ...
                      <span className="font-mono text-[11px] font-medium underline underline-offset-2">{att.label}</span>
                    </a>
                  ))}
                </div>
              )}
